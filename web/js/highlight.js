...
hljs.registerLanguage("scheme", function(hljs) {
  ...
  var BUILTINS = {
    'builtin-normal':
      "eval apply boolean? not eq? equal? pair? null? list? length cons car cdr list append string? symbol? number? integer? + - * / quotient modulo remainder floor ceil abs sqrt expt = < > <= >= even? odd? zero? atom? print display error newline exit set-car! set-cdr! unbind if and or define define-macro cond begin else lambda mu let quote unquote quasiquote unquote-splicing set! procedure? cons-stream cdr-stream delay force",
    'builtin-special': "string-append make-string string->symbol vector make-vector vector? vector->list list->vector vector-ref vector-set! hash-code vector-length error-notrace bindings js js-call js-ref js-set! js-has-property? js-object js-object? list-saves download load upload save diagram exit-diagram visualize listen handle wait cancel cancel-all library demo breakpoint debug step continue edit vis-goto vis-next vis-back vis-first vis-last vis-exit listeners theme ui-color ui-border ui-text-style set-custom-css reset-theme create-button delete-button trigger-event draw-pair zoom build-info bound? bound-in-global? hide save-diagram logic logic-query fact query ? ! import export",
    'builtin-turtle': "forward fd backward back bk left lt right rt circle setposition setpos goto setheading seth penup pu pendown pd showturtle st hideturtle ht speed clear color begin_fill end_fill exitonclick exitturtle exit_turtle bgcolor pensize turtle_help internal_canvas external_canvas pixel pixelsize rgb screen_width screen_height save_image"
  };
